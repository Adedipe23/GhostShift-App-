...
class BurnoutViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=False, methods=["post"], url_path="test-calculate")
    def test_calculate(self, request):
        """
        POST /api/burnout/test-calculate/
        Accepts raw metrics and returns score breakdown and recommendations.
        """
        weekly_hours = float(request.data.get("weekly_hours", 0.0))
        consecutive_shifts = int(request.data.get("consecutive_shifts", 0))
        night_shifts = int(request.data.get("night_shifts", 0))
        rest_hours = float(request.data.get("rest_hours", 8.0))

        result = BurnoutService.calculate_score_locally({
